...
def make_model(args):
    ...
    return PWC_Recons(n_colors=args.n_colors, n_sequence=args.n_sequence, extra_RBS=args.extra_RBS,
                      ...
                      HR_in=args.HR_in, flow_pretrain_fn=flow_pretrain_fn)
...
class PWC_Recons(nn.Module):
    ...
    def __init__(self, n_colors=3, n_sequence=5, extra_RBS=5, recons_RBS=10, n_feat=32, kernel_size=3,
                 scale=4, ksize=13, HR_in=False, flow_pretrain_fn='.'):
        ...
        self.flow_net = flow_pwc.Flow_PWC(pretrain_fn=flow_pretrain_fn)
    ...
    def execute(self, input_dict):
        ...
        if mode == 'train':
            ...
            est_kernel = self.kernel_net(jt.concat(frame_list, dim=1))
    ...
    def recons_forward(self, frame_list):
        ...
        if not self.HR_in:
            base = nn.interpolate(base, scale_factor=self.scale, mode='bilinear', align_corners=False)
        ...
        fusion_feat = self.fusion_conv(jt.concat(warped_feat_list, dim=1))
    ...
    def conv_func(self, input, kernel, padding='same'):
        ...
        conv_result_list = []
        for i in range(c):
            conv_result_list.append(nn.conv2d(input[:, i:i + 1, :, :], kernel, bias=None, stride=1, padding=pad))
        conv_result = jt.concat(conv_result_list, dim=1)
        return conv_result
    ...
    def blur_down(self, x, kernel, scale):
        ...
        # blur
        x = nn.pad(x, (psize, psize, psize, psize), mode='replicate')
        blur_list = []
        for i in range(b):
            blur_list.append(self.conv_func(x[i:i + 1, :, :, :], kernel[i:i + 1, :, :, :], padding='valid'))
        blur_result = jt.concat(blur_list, dim=0)

        # down
        down_result = blur_result[:, :, ::scale, ::scale]
        return down_result 