...
class Trainer_Flow_Video(Trainer):
    def __init__(self, args, loader, my_model, my_loss, ckp):
        ...
        if args.load != '.':
            try:
                mid_logs = jt.load(os.path.join(ckp.dir, 'mid_logs.pkl'))
    ...
    def make_optimizer(self):
        ...
        model_params = []
        model_params.extend(list(self.model.get_model().in_conv.parameters()))
        model_params.extend(list(self.model.get_model().extra_feat.parameters()))
        model_params.extend(list(self.model.get_model().fusion_conv.parameters()))
        model_params.extend(list(self.model.get_model().recons_net.parameters()))
        model_params.extend(list(self.model.get_model().upsample_layers.parameters()))
        model_params.extend(list(self.model.get_model().out_conv.parameters()))
        model_params.extend(list(self.model.get_model().kernel_net.parameters()))
        
        # Flow net parameters with different learning rate
        flow_params = list(self.model.get_model().flow_net.parameters())
        
        optimizer = optim.Adam([
            {'params': model_params, 'lr': self.args.lr},
            {'params': flow_params, 'lr': 1e-6}
        ], **kwargs)
