...
class VIDEODATA_ONLINE_REALISTIC(Dataset):
    ...
    def get_lr_blur_down(self, img_gt, kernel, scale):
        ...
        gt_tensor = jt.array(img_gt.transpose(2, 0, 1)).unsqueeze(0).float()

        kernel_size = kernel.shape[0]
        psize = kernel_size // 2
        gt_tensor = nn.pad(gt_tensor, (psize, psize, psize, psize), mode='replicate')
        ...
        nn.init.constant_(gaussian_blur.weight, 0.0)
        gaussian_blur.weight[0, 0, :, :] = jt.array(kernel).float()
        gaussian_blur.weight[1, 1, :, :] = jt.array(kernel).float()
        gaussian_blur.weight[2, 2, :, :] = jt.array(kernel).float()
